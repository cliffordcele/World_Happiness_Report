...
interpreting coefficients, Cohen's recommended guidelines
state the following:
...
All bivariate relationships are summarized in TABLE XXX. The strongest 
...
(with 4 clusters producing an R<sup>2</sup> = 0.54). This also appears to 
be the pointwhere adding new clusters only provides a small increase to the 
...
```
        *Merging all output datasets for all methods into one dataset sorted by the # of clusters;
        ...
        RUN;
```
```{r, echo=FALSE, out.width="65%", fig.cap="Fig. XX R<sup>2</sup> Plots for the Five Different Methods.", fig.show='hold', fig.align='center'}
...
Since several variables are correlated with one another, the typical measures used to evaluate different k-means clustering models, i.e., the Cubic Clustering Criterion (CCC) and overall R<sup>2</sup>, cannot be used. To use them, a principal component analysis (PCA) must be performed to obtain orthogonal axes before using the k-means algorithm.
...
1.  Compute the correlation matrix of the observational data (see TableXX/Figure
    XXX).
2.  Compute the eigenvalues and eigenvectors of the correlation matrix (see Table XXXX).
3.  Determine the number of components to extract.

The Kaiser rule indicates to retain principal component (PCs) whose eigenvalues are greater than one. In this case, the eigenvalues of the first two components are larger than one. The largest component explains 50.67% of the total variance in the data, and the second largest component 19.72%. In total, 70.39% of the total variance in the data can be explained by the two components. In the scree plot (Figure XXX), the "elbow" is located at the position of the second largest eigenvalue, suggesting the first two components should be extracted.
...
```{r, echo=FALSE, out.width="33%", fig.cap=" Table. XX Eigenvalues & The Variance Explained.", fig.show='hold', fig.align='center'}
knitr::include_graphics("images/eigenvalues.png")
```


```{r, echo=FALSE, out.width="65%", fig.cap=" Fig. XXX Scree Plot & Variance Explained per PC.", fig.show='hold', fig.align='center'}
...
For this method, the number of clusters used for the cluster analysis, the k in k-means, is determined a priori. Then each observation is assigned to its closest cluster. 

The order of the reassignment of observations and the recalculation of the centroids are determined by the method one selects: classical k-means clustering versus improved k-means clustering method.

This reassignment of observations is repeated until no improvements can be made to the clusters.
...
```
* Conduct k-means cluster anlaysis using the PC scores for values k = 2 to 20;
%MACRO doFASTCLUS;
	%DO k=2 %TO 20;
	  ...
	  * Conduct K-mean;
	  PROC FASTCLUS 
		  DATA=dataset_PCA OUT=dsKM_&k MAXITER=100 RADIUS=0 MAXCLUSTERS=&k SUMMARY REPLACE=full;
		  VAR Prin1 Prin2;
	  RUN;
...
```{r, echo=FALSE, out.width="45%", fig.cap="FIGURE XX. K-Means Evaluation Statistics.", fig.show='hold', fig.align='center'}
...
```
 ...
 - The `CCC` compares the R2 you get for a given set of clusters with the R2 you would get by clustering a uniformly distributed set of points in p dimensional space. Similarly, local maxima of the `CCC` indicate potential cluster solutions (k = 7, 9, 10).
...
- Discuss use of ANOVA on original measures too describle clusters
